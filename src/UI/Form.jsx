...
function FileInput({
  ...
  const [preview, setPreview] = useState(null);
  const [uploadResult, setUploadResult] = useState(null); // For success status
  const [uploading, setUploading] = useState(false);
  ...
  const handleFileChange = async (e) => {
    ...
    if (files && currentFile) {
      ...
      // Simulate file upload and success state (you can add actual upload logic here if needed)
      setUploading(true);

      // Commenting out Cloudinary for now, as per your request
      // const result = await handleUpload(currentFile);
      const result = { url: currentFile.name }; // Simulating upload result
      onChange(currentFile); // Pass the file directly to the backend
      setUploading(false);

      if (result) {
        setUploadResult(result); // Set success result
      }
    }
  };
