    ...
    <div className="py-8">
      <div className="bg-background p-6 rounded-md max-w-[1440px] mx-auto">
        ...
        <div className="flex flex-col space-y-6">
          ...
          {/* Display Timestamps */}
          <div>
            <span className="font-semibold text-primary-dark">Created At:</span>{" "}
            <span className="text-primary">
              {formatDate(currentAdmin.createdAt)}
            </span>
          </div>
          <div>
            <span className="font-semibold text-primary-dark">Updated At:</span>{" "}
            <span className="text-primary">
        ...
        {dataStations && dataStations.length > 0 ? (
          <div className="grid grid-cols-1   sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {dataStations.map((station) => (
              <div
                key={station._id}
                className="p-4 border flex flex-col gap-2 rounded-md shadow-sm bg-white"
              >
                <h4 className="font-semibold text-primary">{station.name}</h4>
                <p className="text-primary-dark">{station.location}</p>
                <p className="text-primary-dark">{station.address}</p>
                <button
          ...
          <p className="text-primary-dark">No service stations available.</p>
        )}
      </div>
    </div>
  );
